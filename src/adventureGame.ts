...
async function main() {
  ...
  var attackDamage: number = 50;
  var numHealthPotions: number = 3;
  var healthPotionsAmount: number = 30;
  var healthPotionDropChance: number = 50;

  //   Game Variables
  var enemies: string[] = ["Skeleton", "Zombie", "Warrior", "Assassin"];
  var maxenemyHealth: number = 75;
  var enemyAttackDamage: number = 25;
  var runnung: boolean = true;
  ...
  GAME: while (runnung) {
    ...
    while (enemyHealth > 0) {
      ...
      let input: string = await mainfunc();
      console.log(input);
      if (input == "attack") {
        ...
        console.log(
          "\t> You strike the " + enemy + " for " + damageDelt + " damage."
        );

        console.log("\t> You  recieve " + damageTaken + " in retaliation!");
        if (health < 1) {
          console.log(
            "\t You have taken too much damage, you are tooo weak to go on!"
          );
          break;
        }
        ...
        if (numHealthPotions > 0) {
          if (health < 100) {
            ...
            console.log(
              "\t> You drink a health potiion, healing yourself for " +
                healthPotionsAmount +
                "\n\t>You now have " +
                health +
                "HP." +
          ...
          console.log(
            "\t> No more health potions available! Defeat enemies for a chance to get one "
          );
        }
      } else if (input == "Run!") {
    ...
    let input: string = await askAgain();
